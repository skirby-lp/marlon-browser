function windowKit(options) {
    ...
    this.callBacks = ['onConnect', 'onReady', 'onMessageEvent', 'onSkillChange', 'onAgentTextEvent', 'onAgentRichContentEvent', 'onAgentChatState', 'onMessageSent', 'onVisitorTextEvent', 'onVisitorMessage'];
    this.readStates = {
        read: 'READ',
        accept: 'ACCEPT'
    };
    ...
    this.lastSequence = 0;
    this.defaults = {
        apiRequestTypes: ['cqm.SubscribeExConversations', 'ms.PublishEvent', 'GetClock', 'getUserProfile', 'cm.ConsumerRequestConversation', 'ms.SubscribeMessagingEvents', 'InitConnection', 'cm.UpdateConversationField', 'cm.RequestConversationResponse'],
        debug: !1
    };
    ...
    this.ajax = function(options) {
        ...
        request.open(options.type || 'GET', options.url, !0);
        request.send()
    };
    ...
    this.connect = function() {
        ...
        _this.lpUtils[method](_this.options.account).then(function(jwt) {
            _this.lpUtils.getDomain('asyncMessagingEnt').then(function(umsDomain) {
                _this.socket.connect("wss://" + umsDomain + "/ws_api/account/" + _this.options.account + "/messaging/consumer?v=3").then(function(openedSocket) {
    ...
    this.socketOpened = function(socket) {
        ...
        socket.onNotification(socket.onNotificationType('MessagingEvent'), function(body) {
            body.changes.forEach(function(change_detail) {
                ...
                switch (direction) {
                    case 'received':
                        console.log("Change Detail: " + change_detail.event.type);
